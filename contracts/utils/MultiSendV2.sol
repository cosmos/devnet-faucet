...
contract MultiSendV2 {
    ...
    modifier onlyInitialized() {
        require(initialized, "MultiSend: contract not initialized");
        _;
    }

    /**
     * @dev Initialize the contract (replaces constructor for proxy compatibility)
     * @param _owner The owner of the contract
     */
    function initialize(address _owner) external {
        require(!initialized, "MultiSend: already initialized");
        require(_owner != address(0), "MultiSend: owner cannot be zero address");
     ...
     */
    function multiSend(address payable recipient, TokenTransfer[] calldata transfers) external payable onlyOwner onlyInitialized {
    ...
    ) external payable onlyOwner onlyInitialized {
     ...
     */
    function getOwnerBalance(address token) external view onlyInitialized returns (uint256) {
        if (token == address(0)) {
     ...
     */
    function getAllowance(address token) external view onlyInitialized returns (uint256) {
    ...
    /**
     * @dev Get the version of this implementation
     */
