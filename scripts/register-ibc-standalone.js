...
async function getKeys() {
  ...
  const privateKeyHex = secureKeyManager.getPrivateKeyHex();
  const publicKeyBytes = secureKeyManager.getPublicKeyBytes();
  const addresses = secureKeyManager.getAddresses();
  
  return {
    privateKey: privateKeyHex,
    publicKey: publicKeyBytes, // Return Buffer directly
    cosmosAddress: addresses.cosmos.address,
    evmAddress: addresses.evm.address
  };
}

// Get private key bytes from hex string
function getPrivateKeyBytes(privateKeyHex) {
  return Buffer.from(privateKeyHex.replace('0x', ''), 'hex');
}

// Get public key bytes from hex string
function getPublicKeyBytes(publicKeyHex) {
  return Buffer.from(publicKeyHex.replace('0x', ''), 'hex');
}
...
function encodeMsgRegisterERC20(signer, erc20addresses) {
  ...
  // Field 2: erc20addresses (repeated string) - empty for native coins
...
async function createAndSendTransaction(keys, sequence, accountNumber, chainId) {
  console.log('Creating transaction to register IBC tokens...\n');
  
  try {
    // Create the message
    const msgValue = encodeMsgRegisterERC20(keys.cosmosAddress, []); // Empty array for native coins
    ...
    // Create pubkey
    const pubkeyBytes = keys.publicKey; // Already a Buffer
    ...
    const privateKeyBytes = getPrivateKeyBytes(keys.privateKey);
...
async function main() {
  ...
  try {
    ...
    console.log(`Using faucet address: ${keys.cosmosAddress}\n`);
    ...
    // Get account info
    const accountInfo = await getAccountInfo(keys.cosmosAddress);
